...
void QbbNetDevice::DequeueAndTransmit(void) {
    ...
    if (m_node->GetNodeType() == 0) {  // server
        ...
        if (qIndex != -1024) {
            ...
            bool valid = false;                                             // 新增这个valid
            for (uint32_t i = 0; i < m_rdmaEQ->GetFlowCount(); i++) {       // 轮询所有QP，查看谁的可发送时间最先
                Ptr<RdmaQueuePair> qp = m_rdmaEQ->GetQp(i);
                if (qp->GetBytesLeft() == 0 || qp->m_nextAvail <= Simulator::Now()) // 后半段为新增，不会导致能发的QP一直被跳过吗
                    continue;
