...
TypeId RdmaHw::GetTypeId(void) {
    static TypeId tid =
        TypeId("ns3::RdmaHw")
            ...
            .AddAttribute("IrnRtoHigh", "High RTO for IRN", TimeValue(MicroSeconds(1230)),
...
void RdmaHw::Setup(QpCompleteCallback cb) {
    ...
    m_qpCompleteCallback = cb;      // 回调 RdmaDriver::QpComplete
}

// 根据QP的dest IP，随机选择一个rtTable()中的表项。
uint32_t RdmaHw::GetNicIdxOfQp(Ptr<RdmaQueuePair> qp) {
    auto &v = m_rtTable[qp->dip.Get()];
    if (v.size() > 0) {
        return v[qp->GetHash() % v.size()];
    }
    std::cout << "node " << m_node->GetId() << ": RdmaHw::GetNicIdxOfQp() : v.size() should > 0, but is " << v.size() << " now.\n";
...
void RdmaHw::AddQueuePair(uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip,
                          ...
                          int32_t flow_id) {
    // create qp
    Ptr<RdmaQueuePair> qp = CreateObject<RdmaQueuePair>(pg, sip, dip, sport, dport);    // 五元组
    qp->SetSize(size);
    qp->SetWin(win);
...
Ptr<RdmaRxQueuePair> RdmaHw::GetRxQp(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport,
    ...
    if (create) {
        ...
        // uint64_t senderkey = GetQpKey(dip, dport, sport, pg);
        // GetQp(senderkey);
        // init the qp
        q->sip = sip;
        q->dip = dip;
...
uint32_t RdmaHw::GetNicIdxOfRxQp(Ptr<RdmaRxQueuePair> q) {
    ...
    std::cout << "node " << m_node->GetId() << ": RdmaHw::GetNicIdxOfRxQp(Ptr<RdmaRxQueuePair> q) : v.size() should > 0, but is " << v.size() << " now.\n";
    NS_ASSERT_MSG(false, "We assume at least one NIC is alive");
    std::cout << "We assume at least one NIC is alive" << std::endl;
    exit(1);
}
...
int RdmaHw::ReceiverCheckSeq(uint32_t seq, Ptr<RdmaRxQueuePair> q, uint32_t size, bool &cnp) {
    ...
    if (seq == expected || (seq < expected && seq + size >= expected)) {    //意味着可以移动window下界。
        ...
        if (q->ReceiverNextExpectedSeq >= q->m_milestone_rx) {
            q->m_milestone_rx += m_ack_interval;  // if ack_interval is small (e.g., 1), condition is meaningless
        ...
        if (Simulator::Now() >= q->m_nackTimer || q->m_lastNACK != expected) {  // new NACK
            q->m_nackTimer = Simulator::Now() + MicroSeconds(m_nack_interval);      // 每个 m_nack_interval 只回复一次 nack
