...
void UdpServer::StartApplication(void) {
    ...
    // 注册本对象的HandleRead函数，m_socket收到包之后会调用它
    ...
    while ((packet = socket->RecvFrom(from))) {
        ...
        if (packet->GetSize() > 0) {
            ...
            if (packet->PeekPacketTag(fst) && packet->PeekPacketTag(fit)) {
                if (firstUsed.GetSeconds() == 0 && fst.GetType() == FlowStatTag::FLOW_START_AND_END) {      // 单包流
                ...
                } else if (firstUsed.GetSeconds() == 0 && fst.GetType() == FlowStatTag::FLOW_START) {       // 多包流的起始包
                    // Should print Flow start in client side for exec time of flow start
                    firstUsed = Seconds(fst.getInitiatedTime());
                } else if (firstUsed.GetSeconds() != 0 && flow_end_time == 0 && fst.GetType() == FlowStatTag::FLOW_END && m_app_recv_buffer.isComplete(expected_flow_size)) {   // 多包流的尾包
                    NS_LOG_INFO("FLOW END Time: " << Simulator::Now() << " FLOWID " << fit.GetId());
                    flow_end_time = Simulator::Now();
                    lastUsed = Simulator::Now();
                } else if (firstUsed.GetSeconds() != 0 && flow_end_time == 0 && m_app_recv_buffer.isComplete(expected_flow_size) && m_irn_server) {     // IRN receiver
                    NS_LOG_INFO("FLOW END Time: " << Simulator::Now() << " FLOWID " << fit.GetId());
                    flow_end_time = Simulator::Now();
                    lastUsed = Simulator::Now();
